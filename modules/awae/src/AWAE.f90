...
!#endif 
                        ...
                        xhatBar_plane = xhatBar_plane + abs(tmp_Vx_wake(n_wake))*tmp_xhat_plane(:,n_wake)
                           ...
                           tmp_k_mt_interp = delta*InterpBin( r_tmp_plane, p%r, u%k_mt(:,np1,nt), ILo, p%NumRadii ) + deltad*InterpBin( r_tmp_plane, p%r, u%k_mt(:,np,nt), ILo, p%NumRadii ) !( XVal, XAry, YAry, ILo, AryLen )
						         ...
						         if ( (tmp_k_mt_interp > tmp_k_mt) .AND. (p_WkadT_tmp(1) > p%P0_WkAdT(1)) ) THEN
						            ...
						            ! In the low resolution domain, we only use the index=0 of the low resoultion timestep m%VwkadT(:,:,:,:,0,nt)						
   ...
   ! Output for debugging wake-added turbulence
   !open(40, file="k_mt.txt", status="replace", action="write")
	!DO np = 0, maxPln 
	!   DO nr = 0, p%numRadii-1
	!    write (40,'(1(ES20.4))',advance='no') u%k_mt(nr,np,1)
   !   END DO
	! write (40,'(1(x))',advance='yes')
   !END DO
   !close (40)  
   !
   !open(41, file="vx_wake.txt", status="replace", action="write")
	!DO np = 0, maxPln 
	!   DO nr = 0, p%numRadii-1
	!    write (41,'(1(ES20.4))',advance='no') u%Vx_wake(nr,np,1)
   !   END DO
	! write (41,'(1(x))',advance='yes')
   !END DO
   !close (41)     
  ...
  ! Progress the wake-added turbulence field in time  
   if ( p%WkAdT ) then
      ...
      do n_hl=0, p%n_high_low
         call InflowWind_CalcOutput(t+p%DT_low+n_hl*p%DT_high, m%u_IfW_WkAdT, p%IfWwkAdT(0), x%IfWwkAdT(0), xd%IfWwkAdT(0), z%IfWwkAdT(0), OtherState%IfWwkAdT(0), m%y_IfW_WkAdT, m%IfWwkAdT(0), errStat, errMsg)
         if ( errStat >= AbortErrLev ) then
            return
         end if
